...
{
    const dictionary_ptr dictionary = MALLOC(sizeof (dict));
...
void* dict_insert(dictionary_ptr* dd, const void* key, const void* value)
{
    // ReSharper disable once CppLocalVariableMayBeConst
    dictionary_ptr d = *dd;

    // ReSharper disable once CppLocalVariableMayBeConst
    element_ptr e =  MALLOC(sizeof(element));
    if (e == NULL)
    {
        error(error_out_of_memory);
        exit(-1);
...
void* dict_search(const dictionary_ptr d, const void* key)
{
    if (d->key_type == 0)
    {
        const unsigned long h = hash_function(key) % d->size;
        ...
        for (element_ptr* prev = &d->table[h]; *prev != 0; prev = &(*prev)->next)
        {
            if (!stricmp((*prev)->key, key))
            {
                ...
                *prev = e->next;
                ...
                // ReSharper disable once CppPointerConversionDropsQualifiers
                const char* p = e->key;  // NOLINT(clang-diagnostic-cast-qual)
                // ReSharper disable once CppPointerConversionDropsQualifiers
