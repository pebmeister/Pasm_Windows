...
void* pasm_malloc(const size_t size, const char* function, const int line)
{
    ...
    const memory_entry entry = { size, strdup(function), line, malloc(size), 0 };
...
void* pasm_realloc(const void* memory, const size_t size, const char* function, const int line)
{
    ...
    else
    {
        ...
        entry.size = size;
        entry.function = _strdup(function);
        entry.line = line;
