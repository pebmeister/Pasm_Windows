...
symbol_table_ptr look_up_macro_param(const int param_number)
{
    ...
    return dict_search(symbol_dictionary, macro_param_name);
}
...
{ 
    ...
    if (current_scope)
    {
        ...
        symbol_table_ptr tmp_ptr = dict_search(symbol_dictionary, temp_name);
        FREE(temp_name);
        if (tmp_ptr)
        {
    ...
    // ReSharper disable once CppLocalVariableMayBeConst
    symbol_table_ptr tmp_ptr = dict_search(symbol_dictionary, name);
    if (tmp_ptr)
        tmp_ptr->times_accessed++;
    return tmp_ptr;
} 
...
void add_minus_symbol(const char* file, const int line, const int value)
{
    ...
    if (minus_symbol_table_size - minus_symbol_table_index < GROW_THRESHOLD)
    {
        ...
        struct plus_minus_sym* temp_ptr = REALLOC(minus_symbol_table, sizeof(struct plus_minus_sym) * minus_symbol_table_size);
        if (temp_ptr == NULL)
        {
            error(error_out_of_memory);
            exit(-1);
...
void add_plus_symbol(const char* file, const int line, const int value)
{
    ...
    if (plus_symbol_table_size - plus_symbol_table_index < GROW_THRESHOLD)
    {
        ...
        struct plus_minus_sym* temp_ptr = REALLOC(plus_symbol_table, sizeof(struct plus_minus_sym) * plus_symbol_table_size);
        if (temp_ptr == NULL)
        {
            error(error_out_of_memory);
            exit(-1);
...
void pop_macro_params(void)
{
    ...
    macro_stack_entry* macro_stack_param_entry_ptr = macro_params_stack->pop(macro_params_stack->instance);
...
{
    const symbol_table_ptr sym = e->value;
    fprintf(file, "\n");
    fprintf(file, "    name:           %s\n", sym->name);
...
{
    const sym_entry* aa = a;
    const sym_entry* bb = b;
...
void dump_symbols(FILE* file)
{
    ...
    int count = 0;
    sym_entry* entry_array = MALLOC(sz * sizeof(sym_entry));
    if (entry_array == NULL)
    {
        error(error_out_of_memory);
        exit(-1);
    ...
    for (int index = 0; index < symbol_dictionary->size; index++)
    {
        ...
        {
            const symbol_table_ptr sym = element->value;
            if (sym->is_var == false && sym->is_macro_param == false && sym->is_macro_name == false)
            {
                current_entry->name = element->key;
...
void dump_changed_symbols(FILE* file)
{
    ...
    {
        const symbol_table_ptr current_entry = changed_sym_stack->item_at(changed_sym_stack->instance, i);
...
int unresolved_symbol_count(void)
{
    ...
    for (int index = 0; index < symbol_dictionary->size; index++)
    {
        ...
        {
            const symbol_table_ptr sym = elem->value;
