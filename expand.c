...
int expand_data_node(const parse_node_ptr p)
{
    ...
    const int result = expand_node(p->data.data);
    return result;
}
...
struct macro_dict_entry* create_macro_entry(const char* name)
{
    ...
    struct macro_dict_entry* macro_dict_entry = dict_search(macro_dict, name);
    if (macro_dict_entry == NULL)
    {
        macro_dict_entry = MALLOC(sizeof(struct macro_dict_entry));
        if (macro_dict_entry == NULL)
        {
            error(error_out_of_memory);
            exit(-1);
        ...
        dict_insert(&macro_dict, name, macro_dict_entry);
    }
...
int expand_macro_expansion_node(const parse_node_ptr p)
{
    ...
    if (p->macro.macro_params != NULL)
        expand_node(p->macro.macro_params);
...
{
    const expr* fun = dict_search(expand_operator_table_dictionary, (const void*)(p->opr.opr));
...
int expand_node(const parse_node_ptr p)
{
    ...
    const expr* fun = dict_search(expand_table_dictionary, (const void*)p->type);
...
static dictionary_ptr create_op_table_dictionary(op_table table[], const size_t size)
{
    ...
    {
        dict_insert(&dictionary, (void*)table[i].tag, &table[i].function);
    }
    return dictionary;
}
